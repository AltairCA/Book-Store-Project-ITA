...
<div align="center">
	...
	<div  id="maindiiv" align="center">
    	<div id="mainnav" align="center" >
         ...
         <table border="0">
         <div>
         <th style="font-style:italic;text-decoration:underline;font-size:30px;padding-bottom:50px;" ><b><u>Frequently Asked Questions</u></b></th>
         ...
         <?php
		 		...
		 		$id_query = mysqli_query($con,"SELECT *,`id` FROM  `q_assist` `bo` where `id` in (select `id` from `q_answers` `qa` where `qa`.`id` = `bo`.`id`) ORDER BY (`bo`.`count`) DESC LIMIT 0 , 20 ");
				$row_id;
				
				while($row_id = mysqli_fetch_array($id_query)){
					
					$q_id = $row_id[0];
					
					
					echo "<tr ><td style='padding-left: 100px;min-width:1160px;' id='$q_id'><div onClick="."changer($q_id,'".$q_id."u')"." style='margin:0px;padding:0px;'><b>$row_id[1]</b></div><ul id='".$q_id."u' style='margin-left: 50px;max-width:1060px;margin-top:0px;'>";
					$q_query = mysqli_query($con,"SELECT * FROM `q_answers` WHERE `id` = $q_id ");
					$row_q;
					while($row_q = mysqli_fetch_array($q_query)){
						echo "<li>$row_q[1]</li>";
					}
					echo "</ul></td></tr>";
