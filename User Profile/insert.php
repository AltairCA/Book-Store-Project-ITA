...
<script>
function success() {
    var ask = window.alert("Successully updated profile!");
    if (ask) {
...
function empty() {
    var ask = window.alert("All fields are empty. Please change at least one field.");
    if (ask) {
...
if(($address != NULL)||($state != NULL)||($city != NULL)||($country != NULL)||($phone != NULL)||($zipcode != NULL))
{
if(($address != NULL)||($address !=""))
{
	$sql1="UPDATE user SET address = '$address' WHERE username ='$username'";
...
if(($state != NULL)||($state !=""))
{
	$sql2="UPDATE user SET state = '$state' WHERE username ='$username'";
...
if(($city != NULL)||($city !=""))
{
	$sql3="UPDATE user SET city = '$city' WHERE username ='$username'";
...
{
	$sql4="UPDATE user SET country = '$country' WHERE username ='$username'";
...
if(($phone != NULL)||($phone !=""))
{
	$sql4="UPDATE user SET phone = '$phone' WHERE username ='$username'";
	if (!mysqli_query($con,$sql4)) {
  	die('Error: ' . mysqli_error($con));
	}
}
...
{
	$sql4="UPDATE user SET zip_code = '$zipcode' WHERE username ='$username'";
	if (!mysqli_query($con,$sql4)) {
  	die('Error: ' . mysqli_error($con));
	}
}
