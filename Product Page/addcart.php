...
$query2=mysqli_query($con,"SELECT* FROM `order` where `username`='$username' and `payment_state`=0");
...
{
 $orderid=$row2['order_id'];
 /*echo "<script>alert('$paystate');</script>";*/
...
mysqli_query($con,"INSERT INTO `order` (`username`,`total_weight`,`total_amount`,`quantity`,`date`,`payment_state`,`package_id`,`address`) VALUES ('$username',0, 0, 0,'$date',0,1,'')");
$query2=mysqli_query($con,"SELECT* FROM `order` where `username`='$username' and `payment_state`=0");
...
$query6 = mysqli_query($con,"select * from `b_o` where `book_id` = $id and `order_id`=$orderid");
if($row6=mysqli_fetch_array($query6)){
	$oldqty = $row6[2];
	$oldqty = intval($oldqty)+1;
	mysqli_query($con,"UPDATE `b_o`
SET `qty`=$oldqty
WHERE `book_id` = $id and `order_id`=$orderid");
	
}else{
	$oldqty = 1;
	
	$query5 = mysqli_query($con,"INSERT INTO `b_o` VALUES ($id,$orderid,$oldqty)");
}



header("Location: ../cart/index.php?qty_update=update");
...
?>		
			...
			<a href=""><span style='position:absolute; top:375px; left:1000px;'><image src=added.png></span></a>
		<!--	<a href=""><span style='position:absolute; top:455px; left:995px;'><image src=button2.png></span></a> -->
			...
			<a href=""><span style='position:absolute; top:375px; left:470px;'>Rating</a>
			<a href=""><span style='position:absolute; top:395px; left:470px;'> <?php 
			 print("$isbn");
			 ?></a>
			<a href=""><span style='position:absolute; top:585px; left:470px;'>Synopsis Description,Synopsis Description</a>
